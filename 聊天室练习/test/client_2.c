...
void C_document(XINXI *YY, int socket_fd)
{
	...
	else if(n == 1)     //文件发送
	{
		...
		strncpy(file_name, basename(file_path), sizeof(file_name));     //获取文件名

		fd = open(file_path, O_RDWR);    //打开文件
		if(fd == -1)
		{
			printf("打开文件失败！！！\n");
			return ;
		...
		sprintf(YY->name, "document_strat");
		send(socket_fd, YY, sizeof(XINXI),0);       //send  YY  将文件信息发送过去
		printf("YY->name = %s\n", YY->name);
		...
		while(1)
		{
			memset(YY->name, 0, sizeof(YY->name));
			memset(YY->buf, 0, sizeof(YY->buf));

			ret = read(fd, YY->buf, sizeof(YY->buf));    //从文件中读出内容
	...
	else if(n == 2)     //文件接收
	{
		...
		read(socket_fd, info, sizeof(info));

		strncpy(file_len, info, sizeof(file_len));
        strncpy(file_name, info + sizeof(file_len), sizeof(file_name));
