...
int main()
{
    ...
    XINXI YY;
    online_node *infohead = (online_node*)malloc(sizeof(online_node));
    online_node *infotail = NULL;

    pthread_mutex_init (&lock, NULL);    //锁
    pthread_cond_init (&ready, NULL);    //条件变量
    ...
    mysql_query(conn,A);
    sprintf(A, "update student set fd=0");      //服务器重启，所有用户都没有连接服务器
    mysql_query(conn, A);
    ...
    while(1)
    {
        ...
        for(i = 0; i < nready; i++)
        {
            ...
            if(ep[i].data.fd == lfd)          //监听套接字有响应，有新的套接字要连接服务器
            {
                ...
                online_node *p = (online_node*)malloc(sizeof(online_node));
                //printf("fd = %d\n",cfd);
                p->fd = cfd;
                p->next = NULL;
                if(infohead->next == NULL)
                {
                    infohead->next = p;
                    infotail = p;
                }
            ...
            else if(ep[i].events & EPOLLIN)
            {
                ...
                if(n == 0)         //客户端关闭
                {
                    ...
                    while(p)
                    {
                        ...
                        {
                            sprintf(A, "update student set zt = 0 where id = %d", p->id);
                            mysql_query(conn,A);        //用户退出，将状态设置为不在线
                            sprintf(A, "update student set fd = 0 where id = %d", p->id);
                    ...
                    while(p)
                    {
                        if(sfd == p->fd)
                        {
                            ...
                            else
                            {
                                ...
                                //printf("JJJJJJJJJJJJ\n");
                                //TX(&(p->YY));
                                pthread_create(&thid[j], NULL, thread_account, &(p->YY));
                                //pthread_cond_wait (&ready, &lock); // lock status = unlock
                            }
                            break;
                        }
                        p = p->next;
...
void Document(XINXI *YY, int sfd)
{
    ...
    printf("YY->ice_2 = %d\n", YY->ice_2);
    printf("YY->name = %s\n", YY->name);
    int ret, fd, y_fd, ZT,recv_len, write_len;
    char file_len[16], file_name[128], buf[1024],file_new_name[128],sign[10],info[200];
    char A[100];

    sprintf(A, "select fd from student where id = %d", YY->y_id);
    mysql_query(conn,A);
    MYSQL_RES *res_ptr;
    MYSQL_ROW  res_row;
    res_ptr = mysql_store_result(conn);
    ...
    if(YY->ice_2 == 71)        //接收来自客户端的文件
    {
        ...
        if(strcmp(YY->name, "document_strat") == 0)
        {
        ...
        else
        {
                ...
                //printf("ret = %d\n", ret);
                if(strcmp(YY->name, "ok") == 0)
                {
                    close(YY->ice_4);
                    printf("文件接收成功！！！\n");
                }

                write(YY->ice_4, YY->buf, YY->zt);
    ...
    else if(YY->ice_2 == 72)
    {
        ...
        printf("YY->m_id = %d\n", YY->m_id);
        sprintf(A, "update student set sj = 1 where id = %d", YY->m_id);
        printf("A = %s\n", A);
        mysql_query(conn,A);       //客户端已准备好接收文件

        recv(sfd, sign, sizeof(sign), 0);
        if(strcmp(sign, "over") == 0)
        {
            sprintf(A, "update student set sj = 0 where id = %d", YY->m_id);
            mysql_query(conn,A);
        }
    }
...
int G_ADD(XINXI *YY, DENN *XX)
{
    ...
    if(res_row == NULL)
    {
        ...
        //printf("id = %d\n", XZ->id); 
        sprintf(A,"select * from %sgrouptable where id = %d", getname_from_id(XX->id), YY->q_id);
        ret = mysql_query(conn,A);
        res_ptr = mysql_store_result(conn);
        res_row = mysql_fetch_row(res_ptr);
        mysql_free_result(res_ptr);
...
int  denglu(XINXI *YY,int sfd)            //登陆
{
    ...
    else
    {
        ...
        else
        {
            ...
            mysql_query(conn, A);
            sprintf(A, "update student set fd=%d where id = %d", sfd,YY->m_id);   //连接的套接字
            mysql_query(conn,A);
            strncpy(B,"登陆成功",50);
            ret = 1;
        }
